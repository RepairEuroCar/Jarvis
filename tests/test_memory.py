...
async def test_remember_and_query(temp_memory_file):
    ...
    await mem.remember("test.key", {"val": 42}, category="test")
...
async def test_forget(temp_memory_file):
    mem = MemoryManager(str(temp_memory_file))
    await mem.remember("delete.me", "gone")
    assert mem.query("delete.me") is not None
    assert await mem.forget("delete.me") is True
...
async def test_save_and_load(temp_memory_file):
    mem = MemoryManager(str(temp_memory_file))
    await mem.remember("persist.test", 123)
    await mem.save()
...
async def test_search_and_auto_save(temp_memory_file):
    ...
    await mem.remember("foo.bar", 1)
    await mem.remember("foo.baz", 2)

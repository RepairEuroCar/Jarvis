...
async def test_invalid_inputs_rejected(monkeypatch):
    ...
    assert "Invalid arguments" in await kali_tools.bruteforce_ssh("1.2.3.4", "users.txt", "pass.txt", "--opt;")
    assert "Invalid target or options" in await kali_tools.run_sqlmap("http://test/;", "")
    assert "Invalid resource script" in await kali_tools.run_msfconsole("bad.sh;")
    assert "Invalid options" in await kali_tools.run_burpsuite(";--bad")

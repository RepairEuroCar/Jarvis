...
import sys
import logging
from datetime import datetime # Не используется напрямую, но может быть полезна для логирования
...
async def main():
    ...
    jarvis_instance = None # Инициализируем None на случай ошибки в конструкторе Jarvis
    try:
        ...
        if jarvis_instance and hasattr(jarvis_instance, 'memory') and hasattr(jarvis_instance, 'save_memory'):
            ...
            try:
                ...
                logging.error(f"❌ Ошибка при сохранении памяти: {save_error}", exc_info=True)
        elif jarvis_instance:
            logging.warning("⚠️ Экземпляр Jarvis существует, но отсутствует метод save_memory или атрибут memory.")
        else:
            logging.warning("⚠️ Экземпляр Jarvis не был создан, сохранение памяти невозможно.")

...
An optional module allows Jarvis to use a PostgreSQL database. Initialise the schema using `docs/jarvis_users_pg.sql` **and** `docs/jarvis_topics_pg.sql` and install `asyncpg` from `requirements.txt`. Load the module via `postgres_interface.load_module` to enable the `list_pg_users` command.

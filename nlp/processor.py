...
class NLUProcessor:
    def __init__(self, memory_manager: Optional[Any] = None, max_history_size: int = 100):
        self.memory_manager = memory_manager
        self.command_patterns: List[CommandPattern] = self._initialize_command_patterns()
    ...
    async def _process_text(self, text_original: str, text_lower: str) -> ProcessingResult:
    ...
    async def _extract_entities(self, pattern: CommandPattern, 
                              text: str, 
                              trigger: str) -> ProcessingResult:
    ...
    def add_pattern(self, intent: str, trigger: str, entity_type: Optional[str] = None, persist: bool = False) -> None:
        """Добавляет новый шаблон распознавания команд."""
        pattern = CommandPattern(
            intent=intent,
            triggers=[trigger],
            entity_extraction_mode=EntityExtractionMode.ALL_AFTER_TRIGGER if entity_type else EntityExtractionMode.NO_ARGS,
    ...
    def _load_custom_patterns(self) -> None:
        ...
        for pat in stored:
            try:
                mode = EntityExtractionMode[pat.get("entity_extraction_mode", "NO_ARGS")]
                cp = CommandPattern(
                    intent=pat["intent"],

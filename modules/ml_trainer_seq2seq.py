...
from transformers import AutoModelForSeq2SeqLM, AutoTokenizer, PreTrainedModel, PreTrainedTokenizerFast, DataCollatorForSeq2Seq

from typing import Any, Dict, Optional, List, Tuple
# Предполагается, что CommandInfo и CommandCategory определены в основном файле Jarvis
try:
    from jarvis.app import CommandInfo, CommandCategory
except ImportError:
...
class CustomSeq2SeqDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: PreTrainedTokenizerFast, max_source_length: int, max_target_length: int, source_prefix: Optional[str] = None):
        ...
        try:
                ...
                for line_num, line in enumerate(f):
                    try:
                        ...
                        if source and target:
                            ...
                            logger.warning(f"Пропущена запись в строке {line_num+1} в {data_path}: отсутствует source_text или target_text.")
                    except json.JSONDecodeError:
                        logger.warning(f"Ошибка декодирования JSON в строке {line_num+1} в {data_path}. Строка пропущена.")
            
            if not self.source_texts:
                logger.error(f"Данные не загружены или файл {data_path} пуст/некорректен.")
            ...
            logger.error(f"Непредвиденная ошибка при загрузке данных из {data_path}: {e}")
            raise
...
class Seq2SeqTrainer:
    def __init__(self, jarvis_instance: Any, config: Dict[str, Any]):
        ...
        self.source_prefix = config.get("source_prefix", None) # Например, "translate English to German: "

        try:
            self.tokenizer: PreTrainedTokenizerFast = AutoTokenizer.from_pretrained(self.tokenizer_name)
            self.model: PreTrainedModel = AutoModelForSeq2SeqLM.from_pretrained(self.model_name).to(self.device)
        except Exception as e:
            logger.error(f"Ошибка загрузки модели или токенизатора ({self.model_name}): {e}")
            raise
        ...
        tensorboard_log_dir = config.get("tensorboard_log_dir", os.path.join(self.checkpoint_dir, "runs", config.get("trainer_id", "default_trainer")))
        os.makedirs(tensorboard_log_dir, exist_ok=True)
        self.writer = SummaryWriter(log_dir=tensorboard_log_dir)

        self.max_source_length = int(config.get("max_source_length", 128))
        self.max_target_length = int(config.get("max_target_length", 128)) # Отдельно для цели
        ...
        self.dataloader_num_workers = int(config.get("dataloader_num_workers", 0)) # Для DataLoader
        ...
        self.train_loader: Optional[DataLoader] = self._get_dataloader(config.get("train_data_path"), "train")
        self.val_loader: Optional[DataLoader] = self._get_dataloader(config.get("val_data_path"), "val")
        self.test_loader: Optional[DataLoader] = self._get_dataloader(config.get("test_data_path"), "test")
    ...
    def _get_dataloader(self, data_path: Optional[str], split: str) -> Optional[DataLoader]:
        if not data_path:
            logger.warning(f"Путь к данным для '{split}' не указан. DataLoader не создан.")
            return None
        ...
        try:
            ...
            if len(dataset) == 0:
                 logger.warning(f"Датасет для '{split}' ({data_path}) пуст. DataLoader не будет эффективен.")
            ...
            logger.error(f"Ошибка при создании DataLoader для '{split}' ({data_path}): {e}")
            return None


    def _prepare_optimization_parameters(self):
        if self.model is None:
            logger.error("Модель не инициализирована. Невозможно подготовить параметры оптимизации.")
            return
        ...
        num_training_steps = self.num_epochs * (len(self.train_loader) if self.train_loader else 1)
        self.scheduler = optim.lr_scheduler.CosineAnnealingLR( # или другой шедулер, например, get_linear_schedule_with_warmup
            self.optimizer,
            T_max=num_training_steps, 
            eta_min=self.config.get("scheduler_eta_min", 1e-7) # Меньшее значение для eta_min
        )
    ...
    def _create_checkpoint(self, epoch: int, val_loss: float):
        if self.model is None or self.optimizer is None or self.scheduler is None:
            logger.error("Модель, оптимизатор или планировщик не инициализированы. Чекпоинт не создан.")
            return
        
        checkpoint_path = os.path.join(self.checkpoint_dir, f"checkpoint_epoch_{epoch}_valloss_{val_loss:.4f}.pt")
    ...
    def _load_checkpoint(self, checkpoint_path: str) -> Tuple[int, float]:
        ...
        try:
            ...
            if self.model_name != checkpoint.get('model_name') or self.tokenizer_name != checkpoint.get('tokenizer_name'):
                logger.warning("Модель или токенизатор в чекпоинте отличаются от текущих. "
            ...
            self.global_step = checkpoint.get('global_step', 0) # И глобальный шаг, если он сохранялся

            val_loss = checkpoint.get('val_loss', float('inf'))
            
            logger.info(f"Чекпоинт успешно загружен. Эпоха: {self.current_epoch}, Val Loss: {val_loss:.4f}")
    ...
    def _calculate_metrics(self, logits: torch.Tensor, targets: torch.Tensor) -> Dict[str, float]:
        if logits is None or targets is None or logits.numel() == 0 or targets.numel() == 0:
            ...
            return {'accuracy': 0.0, 'perplexity': float('inf'), 'loss': float('inf')} # Возвращаем дефолтные значения
        ...
        mask = (targets_cpu != self.tokenizer.pad_token_id) & (targets_cpu != -100) # -100 тоже игнорируем
        
        masked_predictions = torch.masked_select(predictions_ids_cpu, mask)
        masked_targets = torch.masked_select(targets_cpu, mask)

        accuracy = (masked_predictions == masked_targets).float().mean().item() if masked_targets.numel() > 0 else 0.0
        ...
        try:
            ...
            logger.warning(f"Ошибка при вычислении perplexity: {e}. Perplexity будет inf.")
    ...
    def _log_metrics(self, metrics: dict, prefix: str, epoch: int, step: Optional[int] = None):
        log_step = step if step is not None else (epoch + 1) * (len(self.train_loader) if self.train_loader and prefix=="train" else 1)

        for name, value in metrics.items():
            if value is not None and not (isinstance(value, float) and (torch.isinf(torch.tensor(value)) or torch.isnan(torch.tensor(value)))):
    ...
    async def train_epoch_async(self, dataloader: DataLoader, current_epoch: int):
        if self.model is None or self.optimizer is None or dataloader is None:
            logger.error("Модель, оптимизатор или DataLoader не инициализированы для обучения эпохи.")
            await self.jarvis.publish_event("ml_training_error", error="Epoch training initialization error")
            return float('inf')
        ...
        try: # Для tqdm, если установлен
            from tqdm.asyncio import tqdm # Используем tqdm.asyncio для асинхронного цикла
            progress_bar = tqdm(enumerate(dataloader), total=len(dataloader), desc=f"Epoch {current_epoch + 1} Training")
        except ImportError:
            logger.info("Библиотека tqdm не найдена. Прогресс-бар не будет отображаться.")
        ...
        for batch_idx, batch in progress_bar:
            ...
            if loss is None: 
                logger.error(f"Loss is None на батче {batch_idx}. Проверьте модель и данные.")
                await self.jarvis.publish_event("ml_training_error", error="Loss is None during training")
                continue
            ...
            if self.log_interval > 0 and (batch_idx % self.log_interval == 0 or batch_idx == len(dataloader) - 1):
                ...
                self._log_metrics(batch_metrics, "train_batch", current_epoch, self.global_step)
                await self.jarvis.publish_event("ml_training_batch_end", epoch=current_epoch, batch=batch_idx, metrics=batch_metrics)
            
            self.global_step += 1
            if isinstance(progress_bar, tqdm): # Обновляем описание прогресс-бара
                 progress_bar.set_postfix(loss=f"{current_loss_item:.4f}", avg_loss=f"{total_loss / (batch_idx + 1):.4f}")
            
            # Освобождаем память GPU, если возможно (осторожно, может замедлить)
            # del inputs, attention_mask, labels, outputs, loss
            # if self.device.type == 'cuda': torch.cuda.empty_cache()

        avg_epoch_loss = total_loss / len(dataloader) if len(dataloader) > 0 else float('inf')
        logger.info(f"Эпоха обучения {current_epoch + 1} завершена. Средний Loss: {avg_epoch_loss:.4f}")
    ...
    async def validate_epoch_async(self, dataloader: DataLoader, current_epoch: int):
        if self.model is None or dataloader is None:
            logger.error("Модель или DataLoader не инициализированы для валидации эпохи.")
            await self.jarvis.publish_event("ml_validation_error", error="Epoch validation initialization error")
            return float('inf')
        ...
        try:
            ...
            progress_bar_val = tqdm(dataloader, total=len(dataloader), desc=f"Epoch {current_epoch + 1} Validation")
        except ImportError:
        ...
        with torch.no_grad():
            for batch in progress_bar_val:
                ...
                epoch_predictions_logits_list.append(outputs.logits.detach().cpu()) # Перемещаем на CPU сразу
        ...
        all_logits = torch.cat(epoch_predictions_logits_list, dim=0).to(self.device) # Возвращаем на device для метрик, если нужно
        ...
        await self.jarvis.publish_event("ml_validation_epoch_end", epoch=current_epoch, metrics=metrics)
        logger.info(f"Эпоха валидации {current_epoch + 1} завершена. Средний Loss: {avg_epoch_loss:.4f}")
        return avg_epoch_loss
    ...
    async def train_async(self):
        ...
        await self.jarvis.publish_event("ml_training_start", epochs=self.num_epochs, config=self.config)

        start_epoch = self.current_epoch # Если загрузили чекпоинт, начинаем с сохраненной эпохи + 1
        ...
        for epoch_idx in range(start_epoch, self.num_epochs):
            ...
            train_loss = await self.train_epoch_async(self.train_loader, self.current_epoch)
            val_loss = await self.validate_epoch_async(self.val_loader, self.current_epoch)

            await self.jarvis.publish_event("ml_training_epoch_end", epoch=self.current_epoch, train_loss=train_loss, val_loss=val_loss)

            if val_loss < best_val_loss:
                best_val_loss = val_loss
                early_stopping_counter = 0
                self._create_checkpoint(self.current_epoch, val_loss) # Сохраняем с текущей эпохой
                logger.info(f"Новый лучший val_loss: {best_val_loss:.4f}. Чекпоинт сохранен для эпохи {self.current_epoch + 1}.")
            else:
                early_stopping_counter += 1
                logger.info(f"Val_loss не улучшился ({val_loss:.4f} vs best {best_val_loss:.4f}). Счетчик early stopping: {early_stopping_counter}/{self.patience}")
                if early_stopping_counter >= self.patience:
                    logger.info(f"Ранняя остановка на эпохе {self.current_epoch + 1}. Val_loss не улучшался {self.patience} эпох.")
                    await self.jarvis.publish_event("ml_training_early_stop", epoch=self.current_epoch, best_val_loss=best_val_loss)
                    break
        
        final_epochs_completed = self.current_epoch + 1
        logger.info(f"Обучение завершено. Всего эпох: {final_epochs_completed}, лучший val_loss: {best_val_loss:.4f}")
        await self.jarvis.publish_event("ml_training_end", best_val_loss=best_val_loss, epochs_completed=final_epochs_completed)
        return {"status": "completed", "best_val_loss": best_val_loss, "epochs_completed": final_epochs_completed}
    ...
    async def evaluate_async(self, test_loader: Optional[DataLoader] = None):
        ...
        metrics = self._calculate_metrics(all_logits, all_targets)
        avg_loss = total_loss / len(loader_to_use) if len(loader_to_use) > 0 else float('inf')
    ...
    async def predict_async(self, text: str, max_length: Optional[int] = None) -> str:
        ...
        self.model.eval()
        eff_max_length = max_length if max_length is not None else int(self.config.get("predict_max_length", 50))
        ...
        try:
            ...
            generation_params = {
                ...
                "repetition_penalty": float(self.config.get("predict_repetition_penalty", 1.0)),
                "length_penalty": float(self.config.get("predict_length_penalty", 1.0)),
                "no_repeat_ngram_size": int(self.config.get("predict_no_repeat_ngram_size", 3)) # Часто полезно
            }
            ...
            logger.info(f"Предсказание для '{text}' (с префиксом '{self.source_prefix if self.source_prefix else ''}'): '{decoded_text}'")
            return decoded_text
        except Exception as e:
            logger.error(f"Ошибка во время предсказания для текста '{text}': {e}", exc_info=True)
            return f"Ошибка предсказания: {e}"

    def save_model_local(self, path: Optional[str] = None):
        if self.model is None or self.tokenizer is None:
            logger.error("Модель или токенизатор не инициализированы. Сохранение невозможно.")
    ...
    @classmethod
    def load_model_from_path(cls, path: str, config_overrides: Optional[Dict[str, Any]] = None, jarvis_instance: Optional[Any] = None):
        ...
        if os.path.exists(config_path):
            try:
                ...
                logger.warning(f"Не удалось загрузить trainer_config.json из {path}: {e}. Используются только overrides.")
        
        if config_overrides: loaded_config.update(config_overrides)
        ...
        try:
            ...
            logger.error(f"Ошибка при создании экземпляра Seq2SeqTrainer из пути {path}: {e}", exc_info=True)
            return None

    def __del__(self):
        if hasattr(self, 'writer') and self.writer:
            try: self.writer.close()
            except Exception as e: logger.warning(f"Ошибка при закрытии TensorBoard SummaryWriter: {e}")
...
async def init_module(jarvis: Any, module_config: Dict[str, Any]):
    ...
    if default_trainer_id:
        ...
        default_config = jarvis.memory.query(f"system.module_configs.ml_trainer_seq2seq.{default_trainer_id}")
        if not default_config and isinstance(module_config.get(default_trainer_id), dict): # Проверяем, есть ли конфиг в module_config
             default_config = module_config.get(default_trainer_id)

        if default_config:
            logger.info(f"Попытка предзагрузки тренера по умолчанию с ID: {default_trainer_id}")
            # Передаем ID как часть конфига, чтобы он сохранился
            default_config.setdefault("trainer_id", default_trainer_id) 
            try:
                active_trainers[default_trainer_id] = Seq2SeqTrainer(jarvis_instance=jarvis, config=default_config)
                logger.info(f"Тренер по умолчанию '{default_trainer_id}' успешно настроен.")
            except Exception as e:
                logger.error(f"Ошибка предзагрузки тренера по умолчанию '{default_trainer_id}': {e}")
        else:
            logger.warning(f"Конфигурация для тренера по умолчанию '{default_trainer_id}' не найдена.")


async def deinit_module(jarvis: Any):
    global active_trainers
    for trainer_name, trainer_instance in list(active_trainers.items()): 
        if hasattr(trainer_instance, '__del__'): trainer_instance.__del__() 
        del active_trainers[trainer_name]
    logger.info(f"Модуль {MODULE_METADATA['name']} деинициализирован, все активные тренеры остановлены.")
...
async def setup_trainer_command(jarvis: Any, args_str: str) -> str:
    ...
    if isinstance(config_from_memory, dict):
        ...
        logger.info(f"Загружена конфигурация тренера '{trainer_id}' из памяти Jarvis: {memory_config_path}")
        ...
        try:
            ...
            logger.info(f"Загружена конфигурация тренера из файла: {config_name_or_path}")
            # Если загрузили из файла, ID может быть внутри файла или равен имени файла
            trainer_id = config_dict.get("trainer_id", os.path.splitext(os.path.basename(config_name_or_path))[0])
        except Exception as e:
    ...
    if not config_dict: return f"Не удалось получить конфигурацию для '{config_name_or_path}'."
    ...
    try:
        if trainer_id in active_trainers:
            logger.warning(f"Тренер с ID '{trainer_id}' уже существует. Пересоздание...")
            # Можно добавить логику закрытия старого тренера
            if hasattr(active_trainers[trainer_id], '__del__'): active_trainers[trainer_id].__del__()

        active_trainers[trainer_id] = Seq2SeqTrainer(jarvis_instance=jarvis, config=config_dict)
        return f"Экземпляр тренера Seq2Seq с ID '{trainer_id}' успешно настроен/пересоздан."
    except Exception as e:
        logger.error(f"Ошибка при создании экземпляра Seq2SeqTrainer с конфигурацией '{config_name_or_path}': {e}", exc_info=True)
        return f"Ошибка создания тренера: {e}"

async def start_training_command(jarvis: Any, args_str: str) -> str:
    trainer_id = args_str.strip()
    if not trainer_id: return "Использование: train_seq2seq <trainer_id>"
    if trainer_id not in active_trainers: return f"Тренер с ID '{trainer_id}' не найден."
    ...
    async def _run_training():
        try:
            ...
            logger.info(msg)
            await jarvis.publish_event("ml_training_task_completed", trainer_id=trainer_id, result=result)
            # Можно отправить сообщение пользователю через Jarvis, если есть такой механизм
        except Exception as e_train:
            logger.error(f"Ошибка в фоновой задаче обучения для '{trainer_id}': {e_train}", exc_info=True)
            await jarvis.publish_event("ml_training_task_error", trainer_id=trainer_id, error=str(e_train))
...
async def predict_command(jarvis: Any, args_str: str) -> str:
    ...
    if len(parts) < 2: return "Использование: predict_seq2seq <trainer_id> <текст_для_предсказания>"
...
async def load_checkpoint_command(jarvis: Any, args_str: str) -> str:
    parts = args_str.split(" ", 1)
    if len(parts) < 2: return "Использование: load_seq2seq_checkpoint <trainer_id> <путь_к_чекпоинту>"
    ...
    except Exception as e: return f"Ошибка загрузки чекпоинта для '{trainer_id}': {e}"
...
async def save_model_command(jarvis: Any, args_str: str) -> str:
    ...
    if not trainer_id: return "Использование: save_seq2seq_model <trainer_id> [путь_для_сохранения]"
    if trainer_id not in active_trainers: return f"Тренер с ID '{trainer_id}' не найден."
...
async def list_trainers_command(jarvis: Any, args_str: str) -> str:
    global active_trainers
    if not active_trainers: return "Нет активных экземпляров ML тренеров."
...
commands_to_register: List[CommandInfo] = [
    CommandInfo(name="setup_seq2seq_trainer", description="Настраивает/пересоздает экземпляр Seq2Seq тренера.", category=CommandCategory.DEVELOPMENT, usage="setup_seq2seq_trainer <trainer_id_или_путь_к_json>", handler_name="setup_trainer_command"),
    CommandInfo(name="train_seq2seq", description="Запускает обучение для Seq2Seq тренера (в фоне).", category=CommandCategory.DEVELOPMENT, usage="train_seq2seq <trainer_id>", handler_name="start_training_command"),
    CommandInfo(name="predict_seq2seq", description="Генерирует текст с помощью Seq2Seq тренера.", category=CommandCategory.UTILITY, usage="predict_seq2seq <trainer_id> <входной_текст>", handler_name="predict_command"),
    CommandInfo(name="load_seq2seq_checkpoint", description="Загружает чекпоинт для Seq2Seq тренера.", category=CommandCategory.DEVELOPMENT, usage="load_seq2seq_checkpoint <trainer_id> <путь_к_чекпоинту.pt>", handler_name="load_checkpoint_command"),
    CommandInfo(name="save_seq2seq_model", description="Сохраняет модель и токенизатор тренера.", category=CommandCategory.DEVELOPMENT, usage="save_seq2seq_model <trainer_id> [путь_для_сохранения]", handler_name="save_model_command"),
    CommandInfo(name="list_seq2seq_trainers", description="Список активных Seq2Seq тренеров.", category=CommandCategory.DEVELOPMENT, usage="list_seq2seq_trainers", handler_name="list_trainers_command")
]

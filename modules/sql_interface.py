...
class Table:
    ...
    async def insert(self, db, data: dict):
        ...
        if 'created_at' in [col['name'] for col in self.columns if col.get('not_null')] and 'created_at' not in data:
            if any(col['name'] == 'created_at' and col['type'].upper() == 'DATETIME' for col in self.columns):
                 data['created_at'] = datetime.datetime.now().isoformat()

        await db.execute(f"INSERT INTO {self.name} ({cols}) VALUES ({placeholders})", tuple(data.values()))
        await db.commit()
        async with db.execute(f"SELECT last_insert_rowid()") as cursor:
            row = await cursor.fetchone()
            return row[0] if row else None

    async def select(self, db, where: dict = None, limit: int = None, columns: list = None, order_by: str = None):
    ...
    async def update(self, db, data: dict, where: dict):
        if not data or not where:
            raise ValueError("Data and where clause cannot be empty for update operation.")
        set_clause = ", ".join([f"{key} = ?" for key in data])
        conditions = " AND ".join([f"{key} = ?" for key in where])
        params = tuple(data.values()) + tuple(where.values())
        await db.execute(f"UPDATE {self.name} SET {set_clause} WHERE {conditions}", params)
        await db.commit()
        ...
        return db.total_changes # This might reflect total changes on the connection since open, or last commit.
    ...
    async def delete(self, db, where: dict):
        ...
        await db.commit()
        return db.total_changes # Similar to update, returns total changes on the connection.
...
async def load_module(jarvis_instance):
    ...
    if not jarvis_instance.sql_db or jarvis_instance.sql_db._closed: # Check if connection exists and is open
    ...
    jarvis_instance.notes_table = Table(
        ...
        [
            ...
            {"name": "created_at", "type": "DATETIME", "not_null": True, "default": "CURRENT_TIMESTAMP"} # Added default
        ]
    )
    await jarvis_instance.notes_table.create(jarvis_instance.sql_db)
    print(f"SQL interface module loaded. Database: {DATABASE_FILE}. Available commands: " + ", ".join(commands.keys()))

async def close_module(jarvis_instance):
    """Closes the database connection when the module is unloaded."""
    if hasattr(jarvis_instance, 'sql_db') and jarvis_instance.sql_db and not jarvis_instance.sql_db._closed:
...
async def add_note_async(jarvis_instance, args: str):
    ...
    if len(parts) == 2:
        ...
        try:
            note_id = await jarvis_instance.notes_table.insert(jarvis_instance.sql_db, note_data)
...
async def view_note_async(jarvis_instance, args: str):
    ...
    try:
        note_id = int(args)
        notes = await jarvis_instance.notes_table.select(jarvis_instance.sql_db, where={"id": note_id})
        if notes:
            note = notes[0]
    ...
    if len(parts) == 3:
        try:
            ...
            if field in ["title", "content"]:
                ...
                existing_notes = await jarvis_instance.notes_table.select(jarvis_instance.sql_db, where={"id": note_id}, limit=1)
...
async def delete_note_async(jarvis_instance, args: str):
    ...
    try:
        ...
        await jarvis_instance.notes_table.delete(jarvis_instance.sql_db, {"id": note_id})

...
from radon.metrics import h_visit, mi_visit # Для метрик Холстеда и индекса обслуживаемости
...
class AdvancedCodeAnalyzer:
    ...
    def __init__(self, jarvis_instance, config=None):
        ...
        self.default_report_format = self.config.get("default_report_format", "markdown")
        self.complexity_threshold_warning = self.config.get("complexity_threshold_warning", 10)
        self.complexity_threshold_critical = self.config.get("complexity_threshold_critical", 20)
        self.max_function_lines = self.config.get("max_function_lines", 75)
        self.max_class_methods = self.config.get("max_class_methods", 20)
        self.ignore_dirs = self.config.get("ignore_dirs", [".venv", "__pycache__", ".git", "node_modules", "build", "dist"])
    ...
    def _get_cached_analysis(self, filepath, analysis_type):
        ...
        if cache_key in self.analysis_cache:
            ...
            if (datetime.datetime.now() - timestamp).total_seconds() / 60 < self.cache_ttl_minutes:
                return data
        return None
    ...
    def _cache_analysis_result(self, filepath, analysis_type, data):
        ...
        if self.cache_results:
            self.analysis_cache[(filepath, analysis_type)] = (datetime.datetime.now(), data)
    ...
    async def get_file_structure_ast(self, filepath):
        ...
        structure = {"functions": [], "classes": [], "imports": [], "filepath": filepath}
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                source_code = f.read()
            ...
            for node in ast.walk(tree):
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    ...
                    lines_in_def = (node.end_lineno - node.lineno + 1) if node.end_lineno else 0
                    is_async = isinstance(node, ast.AsyncFunctionDef)
                    structure["functions"].append({
                        "name": node.name, "async": is_async, "lineno": node.lineno,
                        "args_count": num_args, "docstring": ast.get_docstring(node) is not None,
                    ...
                    structure["classes"].append({
                        "name": node.name, "lineno": node.lineno, "methods_count": len(methods),
                        "docstring": ast.get_docstring(node) is not None, "lines_in_def": lines_in_def
                    })
                elif isinstance(node, ast.Import):
                    for alias in node.names:
                        structure["imports"].append({"type": "import", "module": alias.name, "as": alias.asname})
                elif isinstance(node, ast.ImportFrom):
                    module_name = node.module if node.module else "." * node.level
                    for alias in node.names:
                        structure["imports"].append({
                            "type": "from-import", "module": module_name, 
                            "name": alias.name, "as": alias.asname
                        })
            self._cache_analysis_result(filepath, "structure_ast", structure)
            return structure, None
        except SyntaxError as e:
            return None, f"Синтаксическая ошибка в файле {filepath} на строке {e.lineno}: {e.msg}"
        except Exception as e:
        ...
        try:
            ...
            for item in visitor.functions: # radon < 5.0.0 uses .functions
                ...
                results["functions"].append({
                    ...
                    "lineno": item.lineno, "endline": item.endline, "rank": rank
                })
            ...
            for item in visitor.classes:
                ...
                rank = radon_complexity.rank(item.real_complexity) # Assuming real_complexity for class overall
                complexities.append(item.real_complexity)
                results["classes"].append({
                    "name": item.name, "complexity": item.real_complexity,
                    "lineno": item.lineno, "endline": item.endline, "rank": rank,
                    "methods_avg_complexity": item.average_complexity if hasattr(item, 'average_complexity') else "N/A"
                })

            if complexities:
                results["average_complexity"] = round(sum(complexities) / len(complexities), 2)
            
            self._cache_analysis_result(filepath, "cyclomatic_complexity", results)
            return results, None
        except Exception as e:
            return None, f"Ошибка вычисления цикломатической сложности для {filepath}: {e}"

    async def detect_code_smells(self, filepath, structure_data=None, complexity_data=None):
        ...
        if not structure_data or not complexity_data:
            return smells, "Не удалось получить данные структуры или сложности для анализа запахов."
        ...
        for func_comp in complexity_data.get("functions", []):
            if func_comp["complexity"] >= self.complexity_threshold_critical:
                ...
                continue
            smells["detected_smells"].append({
                "type": "High Cyclomatic Complexity", "severity": severity,
        ...
        if self.config.get("enforce_docstrings", False):
            for func in structure_data.get("functions", []):
                if not func["docstring"]:
                     smells["detected_smells"].append({
                        "type": "Missing Docstring", "severity": "info",
    ...
    async def generate_comprehensive_report(self, path_str, report_format=None):
        ...
        if os.path.isfile(path_str):
            ...
            for root, _, files in os.walk(path_str):
                if self._is_path_ignored(root, path_str):
                    continue
                for file in files:
                    if file.endswith(".py") and not self._is_path_ignored(os.path.join(root, file), path_str) :
                        files_to_analyze.append(os.path.join(root, file))
        else:
            return None, f"Ошибка: Путь не найден или не является файлом/директорией: {path_str}"

        if not files_to_analyze:
            return {"summary": "Нет файлов Python для анализа по указанному пути или все игнорируются.", "files": []}, None

        report_data = {"summary": {}, "files": [], "timestamp": datetime.datetime.now().isoformat()}
        all_metrics_summary = {"sloc": 0, "comment_lines": 0, "blank_lines": 0, "maintainability_index_sum": 0, "analyzed_files_count": 0}
        all_smells_count = 0
        ...
        for filepath in files_to_analyze:
            ...
            smells, err_sm = await self.detect_code_smells(filepath, structure, complexity)
            
            file_report = {"filepath": filepath}
            if metrics: file_report["metrics"] = metrics
            if structure: file_report["structure"] = structure
            if complexity: file_report["complexity"] = complexity
            if smells: 
                file_report["smells"] = smells["detected_smells"]
                all_smells_count += len(smells["detected_smells"])
                critical_smells_count += sum(1 for s in smells["detected_smells"] if s.get("severity") == "critical")

            if metrics and metrics.get("maintainability_index") != "N/A":
                all_metrics_summary["maintainability_index_sum"] += metrics["maintainability_index"]
        ...
        report_data["summary"]["total_comment_lines"] = all_metrics_summary["comment_lines"]
        report_data["summary"]["total_blank_lines"] = all_metrics_summary["blank_lines"]
        if all_metrics_summary["analyzed_files_count"] > 0:
            avg_mi = all_metrics_summary["maintainability_index_sum"] / all_metrics_summary["analyzed_files_count"]
...
async def load_module(jarvis_instance, module_config=None):
    ...
    if not hasattr(jarvis_instance, 'adv_code_analyzer') or jarvis_instance.adv_code_analyzer is None:
        # Передаем весь экземпляр Jarvis, чтобы анализатор мог получить доступ, например, к self.jarvis.memory
        jarvis_instance.adv_code_analyzer = AdvancedCodeAnalyzer(jarvis_instance, module_config)
        print("Модуль Advanced Code Analyzer загружен.")
    else:
        # Возможно, обновить конфигурацию, если модуль уже загружен
        jarvis_instance.adv_code_analyzer.config = module_config if module_config else {}
        print("Модуль Advanced Code Analyzer уже был загружен. Конфигурация обновлена (если предоставлена).")
...
def _format_report(report_data, format_type="markdown"):
    ...
    for file_report in report_data.get("files", []):
        relative_filepath = file_report.get('filepath') # TODO: Сделать путь относительным к корню анализа
        md += f"\n## Файл: `{relative_filepath}`\n"
        
        if "metrics" in file_report:
            md += "### Метрики:\n"
            for k, v in file_report["metrics"].items():
                if k != "filepath": md += f"- {k.replace('_', ' ').capitalize()}: {v}\n"
...
async def analyze_code_report_cmd(jarvis_instance, args_string: str):
    ...
    for part in parts[1:]:
        if part.startswith("--format="):
            ...
            if report_format not in ["json", "markdown", "html"]: # HTML пока не реализован
...
async def analyze_py_complexity_cmd(jarvis_instance, args_string: str):
    ...
    analyzer = jarvis_instance.adv_code_analyzer
    if not analyzer: return "Ошибка: Модуль Advanced Code Analyzer не загружен."
    ...
    if complexity_data.get("functions") or complexity_data.get("classes"):
        ...
        for func in complexity_data.get("functions", []):
            output += (f"Функция: `{func['name']}` (строки {func['lineno']}-{func['endline']})\n"
                       f"  Сложность: {func['complexity']} (Ранг: {func['rank']})\n")
        for cls in complexity_data.get("classes", []): # Если Radon предоставляет агрегированную сложность класса
             output += (f"Класс: `{cls['name']}` (строки {cls['lineno']}-{cls['endline']})\n"
...
async def analyze_py_smells_cmd(jarvis_instance, args_string: str):
    ...
    if smells_data and smells_data["detected_smells"]:
        for smell in smells_data["detected_smells"]:
            output += (f"[{smell['severity'].upper()}] {smell['type']} на {smell['location']}\n"

...
class AnalyticalThoughtProcessor(
    ...
    async def _find_patterns(self, problem_or_data: Union[str, Dict]) -> List[str]:
        ...
        if (
            len(re.findall(r"\d{4}", str(problem_or_data))) > 0
        ):  # Пример: найти 4-значные числа (годы?)
            patterns_found.append(
                "Обнаружены числовые последовательности (возможно, даты/годы)."
            )
        return (
            patterns_found
            if patterns_found
            else ["Значимых паттернов не выявлено (заглушка)."]
        )
    ...
    async def _make_comparisons(
        ...
        if (
            "лучше" in str(problem_or_data).lower()
            and "хуже" in str(problem_or_data).lower()
        ):
            return {
                "comparison_type": "A vs B",
                "result": "Требуется более детальный анализ для сравнения.",
            }
    ...
    async def process(self, problem: str, context: dict) -> dict:
        ...
        analysis_metrics, analysis_patterns, analysis_comparisons = (
            await asyncio.gather(metrics_task, patterns_task, comparisons_task)
        )
...
class Brain:
    ...
    async def think(self, problem: str, context: dict) -> dict:
        ...
        try:
            ...
            logger.error(
                f"Ошибка в процессоре {processor.__class__.__name__}: {e}",
                exc_info=True,
            )
            solution = {
                "error": str(e),
    ...
    async def _classify_problem(self, problem: str, context: dict) -> str:
        ...
        if (
            "проанализируй" in problem_lower
            or "сравни" in problem_lower
            or "оцени" in problem_lower
            or "статистика" in problem_lower
            or "данные по" in problem_lower
        ):
            return "analytical"
        elif (
            "создай" in problem_lower
            or "придумай" in problem_lower
            or "напиши" in problem_lower
            or "сгенерируй" in problem_lower
            or "идея" in problem_lower
        ):
            return "creative"
        elif (
            "как" in problem_lower
            or "почему" in problem_lower
            or "что если" in problem_lower
            or "объясни" in problem_lower
            or "план" in problem_lower
        ):
            return "logical"
...
class Jarvis:
    ...
    def _register_core_commands(self):
        ...
        core_command_infos = [
            ...
            CommandInfo(
                ...
                description="Создает Python функцию с использованием AST.",
                category=CommandCategory.DEVELOPMENT,
                usage="create_python_function <сигнатура функции>",
                aliases=["создай_функцию_ast", "новая_функция_ast"],
            ),
            CommandInfo(
                name="generate_large_python_file",
                description="Генерирует Python файл с большим количеством строк.",
                category=CommandCategory.DEVELOPMENT,
                usage="generate_large_python_file <путь_к_файлу.py> [число_строк]",
                aliases=["gen_long_py"],
            ),
            CommandInfo(
                name="analyze_python_file",
                description="Анализирует Python файл.",
                category=CommandCategory.DEVELOPMENT,
                usage="analyze_python_file <путь_к_файлу.py>",
                aliases=["анализируй_py"],
            ),
            CommandInfo(
                name="reason",
                description="Запускает Мозг для обдумывания проблемы.",
                category=CommandCategory.REASONING,
                usage="reason <описание проблемы>",
            ...
            CommandInfo(
                ...
                usage="load_module <имя_модуля>",
                aliases=["загрузи_модуль"],
            ),
            CommandInfo(
                name="set_project",
                description="Устанавливает текущий проект.",
                category=CommandCategory.PROJECT,
                usage="set_project <путь_к_проекту>",
                aliases=["установи_проект"],
            ),
            CommandInfo(
                name="update_template",
                description="Предлагает обновления шаблонов на основе истории изменений.",
                category=CommandCategory.PROJECT,
                usage="update_template <template_name>",
                aliases=["обнови_шаблон"],
            ),
            CommandInfo(
                name="remember",
                description="Сохраняет информацию в память.",
                category=CommandCategory.MEMORY,
                usage="remember <ключ> <значение>",
                aliases=["запомни"],
            ),
            CommandInfo(
                name="query_memory",
                description="Запрашивает информацию из памяти.",
                category=CommandCategory.MEMORY,
                usage="query_memory <ключ>",
                aliases=["спроси_память", "что_в_памяти"],
            ),
            CommandInfo(
                name="forget",
                description="Удаляет информацию из памяти.",
                category=CommandCategory.MEMORY,
                usage="forget <ключ>",
                aliases=["забудь"],
            ),
            CommandInfo(
                name="teach_pattern",
                description="Добавляет пользовательский NLU шаблон.",
                category=CommandCategory.DEVELOPMENT,
                usage="teach_pattern <intent> <trigger_phrase> [entity_type]",
                aliases=[],
            ),
            CommandInfo(
                name="python_dsl",
                description="Преобразует фразу в код Python.",
                category=CommandCategory.DEVELOPMENT,
                usage="python_dsl <фраза>",
                aliases=["dsl_python"],
            ),
            CommandInfo(
                name="parse_doc",
                description="Извлекает требования из описания.",
                category=CommandCategory.DEVELOPMENT,
                usage="parse_doc <текст>",
                aliases=["разбери_док"],
            ),
            CommandInfo(
                name="self_learn",
                description="Запускает обучение Seq2Seq модели.",
                category=CommandCategory.DEVELOPMENT,
                usage="self_learn <trainer_id>",
                aliases=[],
            ),
            CommandInfo(
                name="self_update",
                description="Коммитит изменения или делает pull.",
                category=CommandCategory.DEVELOPMENT,
                usage="self_update <commit|pull> [args]",
                aliases=[],
            ),
        ]
    ...
    def register_command(self, cmd_info: CommandInfo, handler: Callable):
        ...
        for alias in cmd_info.aliases:
            self.commands[alias.lower()] = (cmd_info, handler)
    ...
    async def handle_user_input(self, text: str, source: str = "cli") -> Optional[str]:
        ...
        if not command_lookup:
            if command_name_from_nlu == "unknown_command" and nlu_result.get(
                "raw_text"
            ):
                logger.info(
                    f"Неизвестная команда, пробую передать '{nlu_result['raw_text'][:50]}...' в Мозг."
                )
        ...
        actual_args = (
            entities
            if cmd_info.name in ["create_python_function", "reason"]
            else entities.get("raw_args", "")
        )
        ...
        try:
            ...
            logger.error(
                f"Ошибка выполнения {cmd_info.name}: {e}\n{traceback.format_exc()}"
            )
    ...
    async def help_command(self, args_str: str) -> str:
        ...
        if filter_name:
            ...
            if not lookup:
                return f"Команда '{filter_name}' не найдена."
            cmd_info, _ = lookup
        ...
        for cmd_info_tuple in self.commands.values():
            ...
            if cmd_info.name not in unique_commands:
                unique_commands[cmd_info.name] = cmd_info
        for cmd_info in sorted(
            unique_commands.values(), key=lambda ci: (ci.category.name, ci.name)
        ):
            categorized_commands[cmd_info.category].append(cmd_info)
        for category, cmds_in_category in sorted(
            categorized_commands.items(), key=lambda item: item[0].name
        ):
            output.append(f"\n--- {category.value} ---")
            for cmd_info in cmds_in_category:
                output.append(
                    f"  {cmd_info.name.ljust(25)} - {cmd_info.description.splitlines()[0]}"
                )
        return "\n".join(output)
    ...
    def _parse_arg_string_to_ast(
        ...
        try:
            ...
            if match_dict:
                ...
                value_type_ast = self._parse_arg_string_to_ast(
                    match_dict.group(2).strip(), "value_type"
                )
            ...
            if re.match(r"^[a-zA-Z_][a-zA-Z0-9_.]*$", arg_str):
                ...
                return ast.Name(id=arg_str, ctx=ast.Load())
            logger.warning(
                f"Не удалось распарсить тип '{arg_str}' для '{arg_name}'. Используется как Name."
            )
            return ast.Name(id=arg_str, ctx=ast.Load())
        except SyntaxError:
    ...
    async def create_python_function_command(self, entities: Dict[str, Any]) -> str:
        ...
        if not name_match:
            return "Ошибка: Не удалось извлечь имя функции."
        func_name_str = name_match.group(1)
        remaining_signature = raw_signature_str[len(func_name_str) :].strip()
        args_str_full = ""
        return_type_str = "None"
        if "->" in remaining_signature:
        ...
        if args_str_full.startswith("(") and args_str_full.endswith(")"):
            args_str_full = args_str_full[1:-1].strip()
        ast_args_list = []
        ast_defaults = []
        imports_needed = set()
        if args_str_full:
            raw_args_list = re.split(
                r",(?![^\[]*\])(?![^\(]*\))(?![^\{]*\})", args_str_full
            )
            for arg_def_str in raw_args_list:
                ...
                if "=" in arg_def_str:
                    name_and_type, default_part_raw = arg_def_str.split("=", 1)
                    default_part = default_part_raw.strip()
                    name_and_type = name_and_type.strip()
                else:
                    name_and_type = arg_def_str
                if ":" in name_and_type:
                    name_part, type_part_raw = name_and_type.split(":", 1)
                    type_part = type_part_raw.strip()
                    name_part = name_part.strip()
                else:
                    name_part = name_and_type.strip()
                arg_name_str = name_part
                arg_type_ast = None
                if type_part:
                    arg_type_ast = self._parse_arg_string_to_ast(
                    ...
                    for el in [
                        ...
                        if re.search(r"\b" + el + r"\b", type_part):
                ...
                if default_part:
                    try:
                        ...
                        logger.warning(
                            f"Ошибка парсинга значения по умолчанию '{default_part}' для '{arg_name_str}'."
                        )
                        arg_default_ast = ast.Constant(value=default_part)
        ...
        for el in [
            ...
            if re.search(r"\b" + el + r"\b", return_type_str):
                imports_needed.add(f"from typing import {el}")
        func_docstring = f"Функция {func_name_str}."
        func_body = [ast.Expr(value=ast.Constant(value=func_docstring)), ast.Pass()]
        function_def_node = ast.FunctionDef(
            ...
            args=ast.arguments(
                ...
                kw_defaults=[],
                kwarg=None,
                defaults=ast_defaults,
            ),
            body=func_body,
            decorator_list=[],
            returns=return_annotation_ast,
        )
        import_nodes = []
        if imports_needed:
            ...
            for module_name, names_list in grouped_imports.items():
                import_nodes.append(
                    ast.ImportFrom(
                        module=module_name,
                        names=[
                            ast.alias(name=n, asname=None)
                            for n in sorted(list(set(names_list)))
                        ],
                        level=0,
                    )
                )
        ...
        try:
            ...
            try:
                ...
                generated_code = astor.to_source(module_ast)
                logger.info("Использован 'astor'.")
            except ImportError:
                logger.error("'astor' не найден.")
                return "Ошибка: ast.unparse и 'astor' не найдены."
    ...
    async def generate_large_python_file_command(self, args_str: str) -> str:
        ...
        await self.publish_event(
            "python_code_generated", type="file", name=filepath, lines=num_lines
        )
    ...
    async def analyze_python_file_command(self, args_str: str) -> str:
        ...
        try:
            tree = ast.parse(source_code, filename=filepath_str)
        except SyntaxError as e:
            return f"Синтаксическая ошибка в '{filepath_str}' строка {e.lineno}, смещение {e.offset}:\n{e.text}"
        except Exception as e:
            return f"Ошибка парсинга '{filepath_str}': {e}"
        ...
        await self.publish_event(
            "python_file_analyzed", filepath=filepath_str, analysis_result=report
        )
        return "\n".join(report)
    ...
    async def reason_command(
        ...
        if isinstance(entities_or_problem_str, dict):
            problem_description = entities_or_problem_str.get(
                "problem_description_entity", ""
            ).strip()
        elif isinstance(entities_or_problem_str, str):
            problem_description = entities_or_problem_str.strip()
        else:
            return "Ошибка: неверный формат аргумента для команды reason."
        if not problem_description:
            return "Пожалуйста, опишите проблему. Использование: reason <описание проблемы>"
        current_context = {
            "user_name": self.user_name,
            "current_timestamp": time.time(),
            "current_project": (
                self.project_manager.current_project.get("name")
                if self.project_manager.current_project
                else "Нет"
            ),
        }
        await self.publish_event(
            "brain_thinking_started",
        ...
        await self.publish_event(
            "brain_thinking_finished",
            problem=problem_description,
            solution_keys=list(solution.keys()),
        )
        try:
            solution_str = json.dumps(
                solution, indent=2, ensure_ascii=False, default=str
            )
        except TypeError:
    ...
    async def load_module_command(self, args_str: str) -> str:
        ...
        if await self.module_manager.load_module(module_name):
            return f"Модуль '{module_name}' загружен."
    ...
    async def update_template_command(self, args_str: str) -> str:
        ...
        if updated:
            self.memory.remember(
                "templates.last_updated", {"template": template_name, "files": updated}
            )
    ...
    async def interactive_loop(self):
        ...
        print(f"\nJarvis vBrain (Python {platform.python_version()})")
        print(f"Пользователь: {self.user_name}")
        if self.project_manager.current_project:
            print(f"Проект: {self.project_manager.current_project['name']}")
        ...
        while self.is_running:
            try:
                ...
                prompt_text = (
                    f"[{self.user_name} @ {p_name}]> "  # Добавлено prompt_text
                )
                user_input = await asyncio.to_thread(sys.stdin.readline)
                user_input = user_input.strip()  # Используем prompt_text
                if not user_input:
                    continue
                ...
                print("\nДля выхода введите 'exit' или Ctrl+D.")
                continue
            except EOFError:
                await self.shutdown()
                break
                ...
                logger.error(
                    f"Критическая ошибка в цикле: {e}\n{traceback.format_exc()}"
                )
                await self.publish_event("on_error", str(e))
...
async def main():
    ...
    try:
        jarvis = Jarvis()
        await jarvis.interactive_loop()
    except Exception as e:
        logger.critical(f"Критическая ошибка: {e}", exc_info=True)
    finally:
        if jarvis and jarvis.is_running:
            logger.info("Аварийное завершение...")
